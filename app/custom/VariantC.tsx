...
export default function VariantC() {
  ...
  const [posA, setPosA] = useState<Pos>({ x: L.ax + 8, y: 8, w: L.fw - 16, h: L.ch - 16 });
  const [posB, setPosB] = useState<Pos | null>(null);
  ...
  const handleFile = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    el.onload = () => {
      ...
      setPosA({ x: L.ax + (L.fw - w) / 2, y: (L.ch - h) / 2, w, h });
      setPosB(null); // reset mirror when new image uploaded
      setImg(el);
      setSel("A");
    };
    el.src = url;
  ...
  const handleMirror = () => {
    // Mirror: same relative position within Side A → same relative position within Side B
    const relX = posA.x - L.ax;
    setPosB({ x: L.bx + relX, y: posA.y, w: posA.w, h: posA.h });
    setSel("B");
  };

  const handleClearB = () => { setPosB(null); setSel("A"); };

  const onTransformA = (e: Konva.KonvaEventObject<Event>) => {
    const n = e.target as Konva.Image;
    setPosA({ x: n.x(), y: n.y(), w: n.width() * n.scaleX(), h: n.height() * n.scaleY() });
    n.scaleX(1); n.scaleY(1);
  };
  const onTransformB = (e: Konva.KonvaEventObject<Event>) => {
    const n = e.target as Konva.Image;
    setPosB({ x: n.x(), y: n.y(), w: n.width() * n.scaleX(), h: n.height() * n.scaleY() });
    n.scaleX(1); n.scaleY(1);
  };
        ...
        >
          <Layer>
            ...
            {!posB && <>
              ...
              <Text x={L.bx} y={L.ch / 2 - 8} width={L.fw}
                text={img ? "← copy to here" : "SIDE B"}
                fontSize={img ? 9 : 10} fill="#ccc" align="center" fontStyle="bold" letterSpacing={img ? 1 : 3} />
            </>}

            {img && <KonvaImage ref={refA} image={img} x={posA.x} y={posA.y} width={posA.w} height={posA.h}
              draggable onClick={() => setSel("A")} onTap={() => setSel("A")}
              onDragEnd={(e) => setPosA(p => ({ ...p, x: e.target.x(), y: e.target.y() }))}
              onTransformEnd={onTransformA} />}

            {img && posB && <KonvaImage ref={refB} image={img} x={posB.x} y={posB.y} width={posB.w} height={posB.h}
              draggable onClick={() => setSel("B")} onTap={() => setSel("B")}
              onDragEnd={(e) => setPosB(p => p ? { ...p, x: e.target.x(), y: e.target.y() } : p)}
              onTransformEnd={onTransformB} />}
      ...
      <div className="flex gap-3 flex-wrap justify-center">
        ...
        {img && !posB && (
          <button onClick={handleMirror}
            ...
            style={{ fontFamily: "var(--font-barlow-condensed)", borderColor: "#f95c05", color: "#f95c05" }}>
            Copy to Side B →
          </button>
        )}
        {posB && (
          <button onClick={handleClearB}
            className="px-6 py-2.5 text-sm font-bold uppercase tracking-widest border-2 transition-colors"
            style={{ fontFamily: "var(--font-barlow-condensed)", borderColor: "#ccc", color: "#999" }}>
            Clear Side B
          </button>
        )}
      </div>
